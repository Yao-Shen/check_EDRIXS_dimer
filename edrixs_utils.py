...
__all__ = ['SparseBinary', 'Sparse4D', 'get_fock_bin_by_N_S', 'two_fermion_S', 'four_fermion_S',
           'calc_XAS_Q0', 'calc_XAS', 'calc_XASmagSq_Q0', 'calc_XASmagSq',
           'calc_RIXS_Q0', 'calc_RIXS', 'calc_RIXSmagSq_Q0',
           'diag', 'diag_n_dipole', 'calc_Q',
...
class SparseBinary:
    ...
    def __init__(self, coordinates, shape=None):
        ...
        shape : 2-tuple of int
            shape of the matrix (default: None).
    ...
    def get_coordinates(self):
        ...
        Returns
        -------
        coordinates : 2d int array
            Coordinates of the nonzero elements.
        """
...
class Sparse4D:
    ...
    def __init__(self, values, coordinates, shape=None, dtype=float):
        ...
        data : 1d array
             Non-zero elements.
        coordinates : 2d 4 x N int array
            Coordinates of non-zero elements.
        ...
        # Determine the shape of the matrix
        default_shape = [np.max(self.coordinates[:,ncoo]) for ncoo in range(4)]
        if shape is not None:
...
def two_fermion_S(emat, lb, rb=None, dtype=float):
    ...
    Parameters
    ----------
    emat: scipy coordinate matrix
        The impurity matrix.
    lb: SparseBinary matrix
        Left fock basis :math:`<F_{l}|`.
    rb: SparseBinary matrix
        Right fock basis :math:`|F_{r}>`.
        rb = lb if rb is None
    
    Returns
    -------
    hmat: scipy coordinate matrix
...
def diag(hmat, emax=None, emin=None, k=None, break_point=False, tol=1e-5, suffix='_i', folder=''):
    ...
    evals{suffix}.npy and evals{suffix}.npy will be saved in the cluster folder.
...
def construct_Tabs(evecs_i, evecs_n, dipole):
    ...
    ----------
    evecs_i : 2d array,
        Eigen vectors of initial states.
    evecs_n : 2d array,
        Eigen vectors of intermediate states.
    dipole : list of sparse matrix,
        Dipolar transition operator
    
    Returns
    -------
    Tabs : list of 2d array,
        Absorption operator
    """
    T_abs = []
    for xyz in range(3):
...
def calc_XAS_Q0(evals_i, evals_n, T_abs,
                Gam_c=0.6, omega=np.linspace(-10,10,1000), pol=[1.0, 1.0, 1.0],
                T=10., tol=1e-3, ngs=None):
    """
    Calculate the x-ray absorption spectroscopy for Q=0 case.
    ...
    Gam_c : float,
        Inverse core-hole lifetime in unit of eV (default: 0.6).
    omega : 1d array,
        Incident energy.
    pol : 3-element list/array,
        Incident x-ray polarization in sample coordinates.
    T : float,
        Temperature in unit of Kelvin (default: 10).
    tol : float,
        Tolerance of bose factor to include the initial states (default: 1e-3).
    ngs : int,
        Number of initial states as ground states. If None it will be determined based on tol (default: None).
    ...
    Example
    -------
    >>> # xas with polarization along sample a direction
    >>> xas = calc_XAS_Q0(evals_i, evals_n, T_abs, Gam_c=0.6, omega=np.linspace(-10,10,1000), pol=[1,0,0], T=40.)
    """
    ...
    pol = np.array(pol) / np.linalg.norm(pol)
    
    # Start calculation
    for nngs, gs_id in enumerate(gs): ## loop over ground states
        F_mag = (T_abs[0, :, gs_id] * pol[0] +
                 T_abs[1, :, gs_id] * pol[1] +
                 T_abs[2, :, gs_id] * pol[2])
        for nomega, om in enumerate(omega):
            xas[nomega] += prob[nngs] * np.sum(np.abs(F_mag)**2 * Gam_c / np.pi /
                                               ((om - (evals_n[:] - evals_i[gs_id]))**2 + Gam_c**2))
    
    return xas

def calc_XAS(evals_i, evals_n, T_abs_all,
             Gam_c=0.6, omega=np.linspace(-10,10,1000), pol=np.array([1.0, 1.0, 1.0]),
             T=10., tol=1e-3, ngs=None):
    """
    Calculate the x-ray absorption spectroscopy for multi-site cluster.

    Parameters
    ----------
    evals_i : 1d array,
        Eigen values for the initial states.
    evals_n : list of 1d array,
        List of eigen values for the intermediate states.
    T_abs_all : list of 2d array,
        List of absorption matrix.
    Gam_c : float,
        Inverse core-hole lifetime in unit of eV (default: 0.6).
    omega : 1d array,
        Incident energy.
    ...
    -------
    >>> # xas with polarization along sample a direction
    >>> xas = calc_XAS(evals_i, evals_n, T_abs_all, Gam_c=0.6, omega=np.linspace(-10,10,1000), pol=[1,0,0], T=40.)
    """
    # Determine the ground state occupation
    GS_energy = np.min(evals_i) ## ground state energy
    beta = 1.0 / 8.6173303E-5 / T
    ...
    # Start calculation
    for evals, T_abs in zip(evals_n, T_abs_all): ## loop over site
        for nngs, gs_id in enumerate(gs): # loop over ground states
            F_mag = (T_abs[0, :, gs_id] * pol[0] +
                     T_abs[1, :, gs_id] * pol[1] +
                     T_abs[2, :, gs_id] * pol[2])
            for nomega, om in enumerate(omega):
                xas[nomega] += prob[nngs] * np.sum(np.abs(F_mag)**2 * Gam_c / np.pi /
                                                   ((om - (evals[:] - evals_i[gs_id]))**2 + Gam_c**2))
    
    return xas

def calc_XASmagSq_Q0(evals_i, evals_n, T_abs,
                     pol=[1.0, 1.0, 1.0], T=10., tol=1e-3, ngs=None):
    """
    Calculate the x-ray absorption spectroscopy squared magnitude for Q=0 case.
    ...
    Returns
    -------
    xas[omega,FmagSq]: 2d array.
    
    Notes
    -----
    omega in xas[omega,FmagSq] is not sorted.

    Example
    -------
    >>> # xas with polarization along sample a direction
    >>> xasmagSq = calc_XASmagSq_Q0(evals_i, evals_n, T_abs, pol=[1,0,0], T=40.)
    """
    # Determine the ground state occupation
    GS_energy = np.min(evals_i) ## ground state energy
    beta = 1.0 / 8.6173303E-5 / T
    ...
    for nngs, gs_id in enumerate(gs): # loop over ground states
        F_mag = (T_abs[0, :, gs_id] * pol[0] +
                 T_abs[1, :, gs_id] * pol[1] +
                 T_abs[2, :, gs_id] * pol[2])
        xas[0].append(evals_n[:] - evals_i[gs_id])
        xas[1].append(prob[nngs] * np.abs(F_mag)**2)
    xas[0] = np.hstack(xas[0])
    xas[1] = np.hstack(xas[1])
    
    return np.vstack(xas)

def calc_XASmagSq(evals_i, evals_n, T_abs_all,
                  pol=[1.0, 1.0, 1.0], T=10., tol=1e-3, ngs=None):
    """
    Calculate the x-ray absorption spectroscopy squared magnitude for multi-site cluster.
    ...
    Returns
    -------
    [xas[omega, magSq]] \times number of sites.

    Example
    -------
    >>> # xas with polarization along sample a direction
    >>> xasmagSq = calc_XASmagSq(evals_i, evals_n, T_abs_all, pol=[1,0,0], T=40.)
    """
    # Determine the ground state occupation
    GS_energy = np.min(evals_i) ## ground state energy
    beta = 1.0 / 8.6173303E-5 / T
    ...
    # Start calculation
    for evals, T_abs in zip(evals_n, T_abs_all): ## loop over sites
        xas_tmp = [[],[]]
        for nngs, gs_id in enumerate(gs): # loop over ground states
            F_mag = (T_abs[0, :, gs_id] * pol[0] +
                     T_abs[1, :, gs_id] * pol[1] +
                     T_abs[2, :, gs_id] * pol[2])
            xas_tmp[0].append(evals[:] - evals_i[gs_id])
            xas_tmp[1].append(prob[nngs] * np.abs(F_mag)**2)
        xas.append(np.vstack([np.hstack(xas_tmp[0]),np.hstack(xas_tmp[1])]))
...
def calc_RIXS_Q0(evals_i, evals_n, T_abs,
                 ...
                 phi=0., thin=15., tth=150., pol=[(0, 0), (0, np.pi / 2.0)], ei=None, ef=None,
    ...
    Gam_c : float,
        Inverse core-hole lifetime in unit of eV (default: 0.6).
    sigma : float,
        Sigma of PseudoVoigt profile.
    fraction : float,
        Fraction of PseudoVoigt profile.
    ...
    ei : 3-element list/array,
        Incident polarization in sample coordinates. If given, it will overwrite thin, tth, pol etc. (default: None).
    ef : 3-element list/array,
        Emission polarization in sample coordinates. If given, it will overwrite thin, tth, pol etc. (default: None).
    T : float,
        Temperature in unit of Kelvin (default: 10).
    tol : float,
        Tolerance of bose factor to include the initial states (default: 1e-3).
...
def calc_RIXS(evals_i, evals_n, T_abs_all,
    ...
    pos : list of 3-element array/list,
        Positions of the sites. If None, assume a chain along a direction (default: None).
    Gam_c : float,
        Inverse core-hole lifetime in unit of eV (default: 0.6).
    sigma : float,
        Sigma of PseudoVoigt profile.
...
def get_tsp(l, m, n):
    ...
    ----------
    l, m, n : float,
        Direction cosines along x, y and z directions.
...
def get_tdd(l, m, n):
    ...
    tdd_sigma[3,1] = tdd_sigma[1,3] = 1.5 * n * l * (l**2 - m**2)
    tdd_pi[3,1]    = tdd_pi[1,3]    = n * l * (1. - 2. * (l**2 - m**2))
    tdd_delta[3,1] = tdd_delta[1,3] = -1. * n * l * (1. - 0.5 * (l**2 - m**2))
    tdd_sigma[3,2] = tdd_sigma[2,3] = 1.5 * m * n * (l**2 - m**2)
    tdd_pi[3,2]    = tdd_pi[2,3]    = -1. * m * n * (1. + 2. * (l**2 - m**2))
    tdd_delta[3,2] = tdd_delta[2,3] = m * n * (1. + 0.5 * (l**2 - m**2))
    tdd_sigma[3,3] = 0.75 * (l**2 - m**2)
    ...
    tdd_sigma[4,1] = tdd_sigma[1,4] = 3. * l**2 * m * n
    tdd_pi[4,1]    = tdd_pi[1,4]    = m * n * (1. - 4. * l**2)
    tdd_delta[4,1] = tdd_delta[1,4] = m * n * (l**2 - 1.)
    tdd_sigma[4,2] = tdd_sigma[2,4] = 3. * l * m**2 * n
    tdd_pi[4,2]    = tdd_pi[2,4]    = l * n * (1. - 4. * m**2)
    tdd_delta[4,2] = tdd_delta[2,4] = l * n * (m**2 - 1.)
